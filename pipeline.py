   ...
   for i,  col in enumerate(X.columns.values):
      ...
      if qNumeric == True and qCate == False:
         for j in range(X[col].shape[0]):
            # tempX.ix[j, col] = X[col][j]*(1+random.randint(-50, 50)*0.001)
            tempX.loc[j, col] = X[col][j]*(1+random.randint(-50, 50)*0.001)
            # print(X[col][j], "->", tempX[col][j])
            # tempX.iat[j, i] = X[col][j]*(1+random.randint(-50, 50)*0.001)
            # X[col][j] = X[col][j]*(1+random.randint(-50, 50)*0.001)
